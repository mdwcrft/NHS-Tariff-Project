...
IF OBJECT_ID(N'dbo.usp_tariffs19') IS NOT NULL
BEGIN
	DROP PROCEDURE dbo.usp_tariffs19
END
GO

IF OBJECT_ID(N'dbo.FINALTariff19') IS NOT NULL
BEGIN
	DROP TABLE dbo.FINALTariff19
END
GO

IF TYPE_ID(N'dbo.HesTableType19') IS NOT NULL
BEGIN
	DROP TYPE dbo.HesTableType19
END
GO

-- CREATE TYPE FOR TVP --
CREATE TYPE dbo.HesTableType19 AS TABLE( 
...
CREATE PROCEDURE dbo.usp_tariffs19
	@TVP HesTableType19 READONLY
	AS
	SET NOCOUNT ON
	CREATE TABLE FINALTariff19(EpisodeID INT, NHSNo VARCHAR(50), Spell INT, Episode INT, Epidur INT, Diag VARCHAR(10), HRG VARCHAR(20), Tariff MONEY)

	INSERT INTO FINALTariff19(EpisodeID, NHSNo, Spell, Episode, Epidur, Diag, HRG, Tariff)
	SELECT
		 EpisodeID
		,newnhsno
		,spell
...
DECLARE @HESTVP AS HesTableType19
...
EXEC dbo.usp_tariffs19 @HESTVP
GO

-- READ DATA --
SELECT * FROM FINALTariff19
ORDER BY EpisodeID